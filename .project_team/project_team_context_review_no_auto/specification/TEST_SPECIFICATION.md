...
| Public API | The four user-callable entry points: `assemble_agent_prompt`, `assemble_constraints_block`, `assemble_phase_prompt`, `create_post_compact_hook`; the `gate(...)` predicate; the `ChatApp` Textual pilot; `ProjectConfig.load/save`; the `mcp__chic__get_agent_info` MCP tool. Tests call these by import, never reach into `_render_*` private helpers except for the explicit purity invariant in scenario 2. |
...
| `post_compact` | The PostCompact SDK hook site (`create_post_compact_hook`). |
