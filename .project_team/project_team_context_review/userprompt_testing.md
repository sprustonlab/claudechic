...
## Drift watch (user-alignment lens)

Three patterns the test suite must NOT introduce:

1. **No structural-floor assertions.** A test asserting "agent always receives constraints segment" would re-introduce a floor we removed. A suppressed segment is correct behavior, not a failure.
2. **No `enabled: false` test for the constraints segment.** That key is not valid for constraints (rejected during spec). Only `scope.sites` controls suppression.
3. **No token-cost assertions.** The user explicitly declined byte-thrift framing. Tests cover correctness, not size.
