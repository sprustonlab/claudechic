...
Start an agent to review our changes. The agent thinks critically and sends back its review using `message_agent`.
...
1. Use `mcp__chic__spawn_agent` to create a review agent with the following characteristics:
   ...
   - Prompt should instruct the agent to:
     ...
     - Use `message_agent("{agent_name}", ...)` to send its findings back to you when done
...
5. After spawning the reviewer, you're done for now. The reviewer will send its findings back to you via `message_agent` when it's ready — no need to check in on it.
...
2. Use `message_agent` to request another review from the same reviewer
...
When done, use `message_agent` to send your review back to the agent named "{agent_name}".
